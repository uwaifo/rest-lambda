...
/*
  ...
  provisioner "local-exec" {
    command = "GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -o ../target/pmspislambdabin -ldflags '-w' ../cmd/prod/main.go"
  }
    // YARDI WS
  provisioner "local-exec" {
    command = "GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -o ../target/pms-ws-yardi-lambda-bin -ldflags '-w' ../pkg/prod/main.go"
  }
...
*/
  // REST API
    provisioner "local-exec" {
    command = "GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -o ../target/pms-rest-api-lambda-bin -ldflags '-w' ../cmd/lambda/main.go"
  }
